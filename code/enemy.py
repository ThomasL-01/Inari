...
class Enemy(Entity):
    ...
    def cooldowns(self):
        ...
        if self.can_get_coin == False and self.statut == 'dead' and not self.coin_14:
    ...
    def coin_update(self):
        coin_val = randint(1, 5)
        if coin_val == 1 and self.have_spawned == False:
            self.coin_cooldown_start = pygame.time.get_ticks()
            self.have_spawned = True
            self.gem = Coin([self.visible_sprites], (self.rect.x+ 10, self.rect.y + 20), self.current_salle, '../graphics/gem')
            self.coin_type = self.gem
            self.gem.add(self.coin_group)
        elif coin_val > 1 and self.have_spawned == False:
            self.coin_cooldown_start = pygame.time.get_ticks()
            self.have_spawned = True
            self.coin = Coin([self.visible_sprites], (self.rect.x + 10, self.rect.y + 20), self.current_salle, '../graphics/coin')
    ...
    def enemy_update(self, player):
        if self.check_death() != True:
            ...
            self.actions(player)
        if self.statut == 'dead' and not self.coin_14:
            self.coin_update()
        self.cooldowns()