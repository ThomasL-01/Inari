...
class Level:    #classe du jeu en entier
    def __init__(self, mdp, current_salle, utilisateur, vie, boss, s1, s2, s3, pot_3, pot_14, pot_nb, coin, gem, tile_size, sound_volume): #recupere les donnees du joueur
        ...
        self.skeleton_music = pygame.mixer.Sound('../inari modif en plus/sounds/Spooky Scary Skeletons.mp3')
        self.door_sound = pygame.mixer.Sound('../inari modif en plus/sounds/door.mp3') 
        self.potion_sound = pygame.mixer.Sound('../inari modif en plus/sounds/potion.mp3')
        self.heal_sound = pygame.mixer.Sound('../inari modif en plus/sounds/heal.mp3')
        self.sound_collect = pygame.mixer.Sound('../inari modif en plus/sounds/coin copie.wav')
        ...
        #Créer la salle
        self.create_salle(import_csv_dispo(salles[self.current_salle]['dispo_sol_mur']))
        self.create_dispo_objet(import_csv_dispo(salles[self.current_salle]['reste'])) 
    ...
    def create_salle(self, dispo): #Créer la disposition des murs et sols de la salle
        ...
        for row_index, row in enumerate(dispo):
            for col_index, col in enumerate(row):
                ...
                if col == '4':
                    sprite = Vide_sol([ self.visible_sprites], '../inari modif en plus/graphics/autres/vide.png', (x+200,y), self.current_salle)
                    self.sprite_group.add(sprite)
                if col == '3':
                    sprite = Vide_sol([ self.visible_sprites], '../inari modif en plus/graphics/autres/sol.png', (x+200,y), self.current_salle)
                    self.sprite_group.add(sprite)
                if col == '2':
                   sprite =  Mur((x+200,y),[self.visible_sprites, self.obstacles_sprites], '../inari modif en plus/graphics/autres/mur.png', self.current_salle)
                   self.sprite_group.add(sprite)
                if col == '1':
                    sprite = Mur((x+200,y),[self.visible_sprites, self.obstacles_sprites], '../inari modif en plus/graphics/autres/mur d.png', self.current_salle)
                    self.sprite_group.add(sprite)
                if col == '0':
                   sprite =  Mur((x+200,y),[self.visible_sprites, self.obstacles_sprites], '../inari modif en plus/graphics/autres/mur g.png', self.current_salle)
                   self.sprite_group.add(sprite)
                ...
                if col != '-1':
                    ...
                    if self.trouve_salle(self.liste_door_g) : 
                        if col == '11':
                            ...
                            self.door_g = Porte([ self.visible_sprites], '../inari modif en plus/graphics/autres/porte g.png' , (x+200,y+30), self.current_salle)
                    else:
                        self.door_g = None
                    if col == '16':
                        #torche 
                    ...
                    if self.trouve_salle(self.liste_door_b):  
                        if col == '14':
                            #port vers le bas
                            self.door_b = Porte([self.visible_sprites], '../inari modif en plus/graphics/autres/porte.png', (x+200,y), self.current_salle)
                    else:
                        self.door_b = None
                    ...
                    if self.trouve_salle(self.liste_door_h):
                        if col == '20':
                            #porte vers le haut
                            self.door_h = Porte([self.visible_sprites], '../inari modif en plus/graphics/autres/porte.png', (x+200,y), self.current_salle)
                    else:
                        self.door_h = None
                    if self.trouve_salle(self.liste_door_d) : 
                        if col == '12':
                            #porte vers la droite
                            self.door_d = Porte([self.visible_sprites], '../inari modif en plus/graphics/autres/porte d.png', (x+200,y+30), self.current_salle)
                    else:
                        self.door_d = None
                    if col == '7':
                        #porte verouillée rouge
                        self.locked_door_red = Locked_porte_rouge([self.visible_sprites], '../inari modif en plus/graphics/autres/porte rouge.png',(x+200,y), self.red_statut, self.current_salle)
                    if col == '6':
                        #porte verouillée verte
                        self.locked_door_green =  Locked_porte_verte([self.visible_sprites], '../inari modif en plus/graphics/autres/porte verte.png',(x+200,y), self.green_statut, self.current_salle)
                    if col == '5':
                        #porte verouillée violette
                        self.locked_door_purp =  Locked_porte_violette([self.visible_sprites], '../inari modif en plus/graphics/autres/porte violette.png',(x+200,y), self.purp_statut, self.current_salle)
                    if col == self.purp_key_spawn and self.purp_statut == 'locked':
                        #clé violette (salle 5)
                        self.clé_purp = Clé_violette([self.visible_sprites, self.key_group], '../inari modif en plus/graphics/autres/cl_violette.png', (x+200,y), self.current_salle)
                    if col == self.red_key_spawn and self.red_statut == 'locked':
                        #clé rouge (salle 32)
                        self.clé_red= Clé_rouge([self.visible_sprites, self.key_group], '../inari modif en plus/graphics/autres/cl_rouge.png', (x+200,y), self.current_salle)
                    if col == self.green_key_spawn and self.green_statut == 'locked':
                        #clé verte (salle 18)
                        self.clé_green = Clé_verte([self.visible_sprites, self.key_group], '../inari modif en plus/graphics/autres/cl_verte.png', (x+200,y), self.current_salle)
                    ...
                    if col == '13' or col == '17':
                        ...
                        if col == '17' and self.boss_statut == 'alive': 
                            ...
                            self.enemy_group.add(self.enemy)
                    if col == '22':
                        Mur((x+200,y),[self.visible_sprites, self.obstacles_sprites], '../inari modif en plus/graphics/autres/baril.png', self.current_salle)
    ...
    def damage_player(self, amount, statut): #Fonction permettant de mettre des dégats au joueur  
        if statut != 'dead':
            if self.player.vulnerable:
                ...
                self.hurt_sound = pygame.mixer.Sound('../inari modif en plus/sounds/hurt copie.wav')
                self.hurt_sound.play()
            if self.player.health <= 25 and self.warning_statut:
                self.warning = pygame.mixer.Sound('../inari modif en plus/sounds/warning copie.wav')
        ...
        if self.can_collide_trader and self.current_salle == 17:
            if self.trader != None: #rajouter un timer pour pouvoir sortir du menu sinon on sera non stop en collision
                if pygame.sprite.collide_rect(self.player, self.trader):
                    self.speech_sound = pygame.mixer.Sound('../inari modif en plus/sounds/Villager.mp3')
    ...
    def cooldowns_verif(self):
        ...
        if self.pause:
            if self.pause_sound_statut:
                self.pause_sound = pygame.mixer.Sound('../inari modif en plus/sounds/pause.wav')
    ...
    def pause_menu(self):
        ...
        x_pos = x // 2
        y_pos = y // 10 *4
        self.title_button = Buttons('../inari modif en plus/graphics/buttons/title/1.png', x_pos, y_pos, '', 'title', x // 2.5, y //10, 'ok', 'menu_button','../inari modif en plus/graphics/buttons/title/2.png' )
        y_pos += y // 10 * 2
        self.continue_button = Buttons('../inari modif en plus/graphics/buttons/continue/1.png', x_pos, y_pos, '', 'continue',x // 2.5, y //10, 'ok', 'menu_button', '../inari modif en plus/graphics/buttons/continue/2.png' )
