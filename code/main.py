...
class Game():
    def __init__(self, utilisateur, mdp, current_salle, vie, boss, s_purp, s_green, s_red, pot_3, po_14, pot_nb,coin,gem,p_stock,strength_stock,stamina_stock,health_stock,speed_stock,res_stock):
        ...
        pygame.mixer.music.load('../inari modif en plus/sounds/music.mp3')
        pygame.mixer.music.play(loops=-1)
        self.screen = pygame.display.set_mode((self.width,self.height), pygame.RESIZABLE)
        image = pygame.image.load('../inari modif en plus/graphics/autres/logo.gif')  
        pygame.display.set_icon(image)
        pygame.display.set_caption('INARI')
        self.clock = pygame.time.Clock()
        self.tile_size = self.screen.get_height() // 15
        
        self.trade_sound = pygame.mixer.Sound('../inari modif en plus/sounds/Trade.mp3')
                    ...
                    for button in self.level.ui.button_list:
                        ...
                        if self.can_hit_button and action != None:                            
                            ...
                            if action == 'title':
                                ...
                                pygame.mixer.music.load('../inari modif en plus/sounds/intro.mp3')
...
def game_over(pseudo, mdp): #Fonction pour afficher le menu game over
    ...
    Go.minsize(480, 360)
    Go.config(bg = 'black')
    lose_music = pygame.mixer.Sound('../inari modif en plus/sounds/Game Over.mp3')
...
def you_win(pseudo, mdp, s1, s2, s3, pot_3, pot_14):  #Fonction pour afficher le menu de victoire
    ...
    win.minsize(480, 360)
    win.config(bg = 'black')
    win_music = pygame.mixer.Sound('../inari modif en plus/sounds/stage_clear.wav')
...
def menu_settings():
    ...
    frame = Frame(window, bg='black')
    volume_frame = LabelFrame(frame, text='VOLUME', font= ("../inari modif en plus/graphics/autres/ARCADEPI.TTF", 25), bg='black', fg='white', bd=0, highlightthickness=0)
