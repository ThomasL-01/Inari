...
def menu_principal():   #genere le menu principal
    ...
    destroy_widgets(widget_list)
    window.minsize(480, 360)
    player_stats['health'] = 100
    player_stats['stamina'] = 200
    player_stats['speed'] = 4
    player_stats['damage'] = 15
    ...
    widget_list.append(can)

    frame = Frame(window, bg='black')
    widget_list.append(frame)

    settings_frame = Frame(window, bg = 'black')
    widget_list.append(settings_frame)


    #creation du sous-titre
    soustitre = Label(frame, text= "ET LE  LABYRINTHE  DE L'INFINI", font= ("Arial", 40), bg = "black", fg = "white")
    soustitre.pack()
    widget_list.append(soustitre)


    #creation des boutons
    image_nouveau_button =PhotoImage(file='../graphics/autres/nouveau.gif').subsample(3)
    nouveau_button = Button(frame, text = "NOUVEAU (Si tu vas sur charger jte donne tout", image=image_nouveau_button, command = menu_nouveau)
    nouveau_button.pack()
    widget_list.append(nouveau_button)

    image_charger_button =PhotoImage(file='../graphics/autres/charger.gif').subsample(3)
    charger_button = Button(frame, text = "Gouloum: pseudo: Thomas /Mot de passe: totodu01", image= image_charger_button, command = menu_charger)
    charger_button.pack(pady=2)
    widget_list.append(charger_button)

    settings_image = PhotoImage(file = '../graphics/settings.gif').subsample(3)
    settings_button = Button(settings_frame, image = settings_image, command = menu_settings)
    settings_button.pack()
    widget_list.append(settings_button)


    #creation bouton quitter
    btn_quitter = Button(window, text="QUITTER", bg="white", fg="black", command=window.destroy)
    btn_quitter.pack(side=BOTTOM, fill=X)
    widget_list.append(btn_quitter)
...
def menu_charger():     #genere le menu pour charger un utilisateur
    ...
    label_pseudo.pack()
    widget_list.append(label_titel)
    widget_list.append(label_pseudo)
    ...
    widget_list.append(erreur_text)


    def entry():#systeme de verification d'une sauvegarde à charger
        pseudo = pseudo_joueur.get()
        mdp = mdp_joueur.get()
        hash_mdp = hashlib.sha256(mdp.encode())  # Encode la chaîne en bytes et calcule le haché
        hex_digest = hash_mdp.hexdigest()  
        erreur = verif_sauvegarde(pseudo, str(hex_digest), 'csvfile.csv')
        if erreur == True:
    ...
    widget_list.append(entrer_button)
    ...
    btn_quitter = Button(window, text="MENU PRINCIPAL", bg="white", fg="black", command=menu_principal)
    btn_quitter.pack(side=BOTTOM, fill=X)
    widget_list.append(btn_quitter)
    window.mainloop()
...
def menu_settings():
    ...
    frame = Frame(window, bg='black')
    widget_list.append(frame)
    volume_frame = LabelFrame(frame, text='VOLUME', font= ("Arial", 25), bg='black', fg='white', bd=0, highlightthickness=0)
    volume_frame.grid(column=0, row=0)
    widget_list.append(volume_frame)
    ...
    volume_slider.pack()
    widget_list.append(volume_slider)

    
    btn_quitter = Button(window, text="MENU PRINCIPAL", bg="black", fg="black", command= menu_principal)
    btn_quitter.pack(side=BOTTOM, fill=X)
    widget_list.append(btn_quitter)
...
def menu_nouveau():     #genere le menu pour creer un nouvel utilisateur
    ...
    height = 700
    width = 700
    image = PhotoImage(file="../graphics/autres/new_user.png", master= window).zoom(1).subsample(2)
    canvas = Canvas(frame, width=width, height=height, bg="black", bd=0, highlightthickness=0 )
    canvas.create_image(width/2, height/2, image= image)
    canvas.grid(row =0, column=0, sticky=W)
    widget_list.append(canvas)
    ...
    def entry(): #systeme de verification/creation d'une nouvelle sauvegarde
        ...
        if verification == True:
            ...
            elif len(str(hex_digest)) < 5:
                error_text.config(fg='red', text='Le mot de passe doit faire 5 caractères minimum')
            else:
                nouvelle_sauvegarde(pseudo, str(hex_digest), 3,100,'alive','locked','locked','locked', 'on ground', 'on ground', 0,0,0,5,4,3,4,4,2)
                destroy_widgets(widget_list)
                window.destroy()
                pygame.mixer.music.stop()
                if __name__ == '__main__':
