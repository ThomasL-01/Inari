...
class Level:    #classe du jeu en entier
    def __init__(self, mdp, current_salle, utilisateur, vie, boss, s1, s2, s3, pot_3, pot_14, pot_nb, coin, gem, tile_size, sound_volume): #recupere les donnees du joueur
        ...
        self.trader_coll_cooldown = 2500
        ...
        if self.current_salle == 20:
            if pygame.sprite.collide_mask(self.player, self.locked_door_red):
                ...
                if self.red_statut == 'open':
                    ...
                    police = pygame.font.Font(ui_pygame_font, ui_font_size)
    ...
    def coin_collision(self, coin_group, start):
        ...
        for coin in coin_group:                
            if coin.type == "coin" and current_time - start >= self.coin_cooldown and pygame.sprite.collide_rect(self.player, coin):
                coin.kill()
            ...
            elif coin.type == "gem" and current_time - start >= self.coin_cooldown and pygame.sprite.collide_rect(self.player, coin):
                coin.kill()
                self.sound_collect.set_volume(self.sound_volume /3)
                self.sound_collect.play()
                self.gem_nb += 1
    ...
    def cooldowns_verif(self):
        ...
        if self.current_salle == 36 and self.boss_statut == 'alive':
            self.ui.show_boss_bar(self.enemy.health, mob_data["boss"]["health"], self.ui.boos_bar_rect)
...
class Enemy_update(pygame.sprite.Group): #classe exprès pour l'update de l'ennemi (pas trouvé d'autre moyen) 
